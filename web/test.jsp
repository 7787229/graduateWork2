...
<div class="content" >
    ...
    <h1><%=test.getTitle()%></h1>
    <ul id="test">
        <li><button type="button" onclick="showWindow($('#change_test'),$('#change_test form'))">Редактировать тест</button></li>
        <li><button type="button" onclick="showWindow($('#add_question'),$('#add_question form'));">Добавить вопрос</button></li>
        <%for (QuestionsEntity question:test.getQuestions()){%>
            <li class="question">
                <label>
                    <span><%=question.getText()%></span>
                </label>
                <button name="change_question" value="<%=question.getId()%>|<%=test.getId()%>">Редактировать</button>
...
<div id="change_question">
    ...
    <form method="post" action="OperationTest">
        <script>
            showWindow($("#change_question"),$("#change_question form"));
        </script>
